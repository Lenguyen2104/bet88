...
const betK3 = async (req, res) => {
    try {
        ...
        if (check >= 0) {
            ...
            let checkTime = timerJoin(new Date().getTime());
            const sql = `INSERT INTO result_k3 SET id_product = ?,phone = ?,code = ?,invite = ?,stage = ?,level = ?,money = ?,price = ?,amount = ?,fee = ?,game = ?,join_bet = ?, typeGame = ?,bet = ?,status = ?, today = ?,time = ?`;
            await connection.execute(sql, [id_product, userInfo.phone, userInfo.code, userInfo.invite, period.period, userInfo.level, total, price, xvalue, fee, game, gameJoin, typeGame, listJoin, 0, checkTime, timeNow]);
            await connection.execute('UPDATE `users` SET `money` = `money` - ? WHERE `token` = ? ', [total, auth]);
            const [users] = await connection.query('SELECT `money`, `level`, `phone`, `ctv` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
            await rosesPlus(auth, total);
            const [level] = await connection.query('SELECT * FROM level ');
            const [idU] = await connection.execute('SELECT `id_user` FROM users WHERE phone = ? LIMIT 1', [ users[0].ctv ]);
...
async function funHanding(game) {
    ...
    for (let i = 0; i < totalN; i++) {
        ...
        // }
        console.log('suilt '+sult);
        let lengWin = sult.filter(function(age) {
            return age == total;
        });
        console.log('lengWin '+lengWin.length);
