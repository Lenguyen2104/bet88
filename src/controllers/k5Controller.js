...
const betK5D = async (req, res) => {
    try {
        ...
        if (check >= 0) {
            ...
            const sql = `INSERT INTO result_5d SET id_product = ?,phone = ?,code = ?,invite = ?,stage = ?,level = ?,money = ?,price = ?,amount = ?,fee = ?,game = ?,join_bet = ?,bet = ?,status = ?, today = ?,time = ?`;
            await connection.execute(sql, [id_product, userInfo.phone, userInfo.code, userInfo.invite, period.period, userInfo.level, total, price, x, fee, game, join, list_join, 0, checkTime, timeNow]);
            ...
            let savedUser = await connection.execute('SELECT * FROM `user` WHERE `token` = ? ', [auth]);
