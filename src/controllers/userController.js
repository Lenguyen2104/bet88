...
const listMyTeam = async(req, res) => {
    ...
    const [total_roses] = await connection.query('SELECT `f1`, `time`, `chitiet` FROM roses WHERE `invite` = ? ORDER BY id DESC LIMIT 100', [userInfo.code]);
...
const addBank = async(req, res) => {
    ...
    if (user_bank.length == 0 && user_bank2.length == 0) {
        ...
        stk = ?,
        time = ?`;
        await connection.execute(sql, [userInfo.phone, name_bank, name_user, stk, time]);
        return res.status(200).json({
...
/*     let tp = req.body.tp;
    ...
    const [check] = await connection.query('SELECT * FROM user_bank WHERE `phone` = ? ', [userInfo.phone]);
    if(check.length > 0){
        let time = new Date().getTime();
        ...
        WHERE phone = ?`;
        try{
        await connection.execute(sql, [name_bank, name_user, stk, userInfo.phone]);
        }catch(err){
            console.log(err);
        }
        return res.status(200).json({
...
const withdrawal3 = async(req, res) => {
  ...
  //      total += data.money;
   ...
   // if(total - total2 > 0) result = total - total2;
    ...
    const [withdraw] = await connection.query('SELECT * FROM withdraw WHERE `phone` = ?', [userInfo.phone]);
