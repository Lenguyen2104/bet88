...
function reload_money() {
    fetch("/api/webapi/GetUserInfo")
        ...
        .then((data) => {
            ...
            $(".num .money_show").text(`${data.data.money_user}.00 ₫ `);
            $(".Loading").fadeOut(0);
        });
}
reload_money();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status) {
                ...
                socket.emit('data-server-3', { chane, gameJoin: 1, listJoin, money, phone: response.phone, xvalue, game: $('html').attr('data-dpr'), ctv: ctv, conlai: response.money });
            }
            dropDown();
        }
    });
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status) {
                $('.money_show').text(response.money + '.00 ₫');
                var ctv = response.ctv;
                socket.emit('data-server-3', { ctv: ctv, chane, gameJoin: 2, listJoin, money, phone: response.phone, xvalue, game: $('html').attr('data-dpr') });
            }
            dropDown();
        }
    });
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status) {
                $('.money_show').text(response.money + '.00 ₫');
                var ctv = response.ctv;
                socket.emit('data-server-3', { ctv: ctv, chane, gameJoin: 3, listJoin, money, phone: response.phone, xvalue, game: $('html').attr('data-dpr') });
            }
            dropDown();
        }
    });
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status) {
                $('.money_show').text(response.money + '.00 ₫');
                var ctv = response.ctv;
                socket.emit('data-server-3', { ctv: ctv, chane, gameJoin: 4, listJoin, money, phone: response.phone, xvalue, game: $('html').attr('data-dpr') });
            }
            dropDown();
        }
    });
